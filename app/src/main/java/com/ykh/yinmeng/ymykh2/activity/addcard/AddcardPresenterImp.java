...
public class AddcardPresenterImp<V extends AddcardMVPView> extends BasePresenter<V> implements AddcardPresenter<V> {
    ...
    public void onInit(Activity activity) {
        ...
        if (cardBean != null){
            ...
            String branch = cardBean.getBranch();
            String[] bankname = branch.split("-");
            String et_bankname = bankname[3];
            String address = branch.substring(0,branch.lastIndexOf("-"));
            getAttachedView().setBranch(et_bankname);
            getAttachedView().setAddress(address);
