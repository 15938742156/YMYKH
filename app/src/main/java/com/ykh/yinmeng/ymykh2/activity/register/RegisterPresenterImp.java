 ...
 * Description：
 */
public class RegisterPresenterImp<V extends RegisterMVPView> extends BasePresenter<V> implements RegisterPresenter<V> {
    ...
    @Override
    public void onToggleRegisterBtn(String yqtel, String tel,String password, String yzm) {
        ServerApi.<LzyResponse>register(yqtel,tel,password, yzm)
                ...
                .subscribe(new Observer<LzyResponse>() {
                    ...
                    public void onNext(LzyResponse response) {
                        if (response.getCode() == 1) {
                            getAttachedView().registerSuccess();
                        } else {
                            getAttachedView().registerFailure(response.getMsg());
                        }
                    }

                    @Override
                    public void onError(Throwable e) {
                        getAttachedView().registerFailure(e.getLocalizedMessage());
    ...
    public void onToggleCodeBtn(String phone) {
        ServerApi.<LzyResponse>getYzm(phone, "2")
                .subscribeOn(Schedulers.io())
                .doOnSubscribe(new Consumer<Disposable>() {
                    @Override
                    public void accept(@NonNull Disposable disposable) {
                        Log.d("udong", "短信登陆获取验证码");
                        getAttachedView().showDialog();
                    }
                })
                .observeOn(AndroidSchedulers.mainThread())
