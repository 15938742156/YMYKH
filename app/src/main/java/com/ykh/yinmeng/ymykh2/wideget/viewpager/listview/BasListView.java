package com.ykh.yinmeng.ymykh2.wideget.viewpager.listview;
...
import com.ykh.yinmeng.ymykh2.adapter.RebateAdapter;

import java.text.SimpleDateFormat;
import java.util.Date;

public class BasListView extends RecyclerView implements AbsListView.OnScrollListener {
    ...
    private RefreshListView.DisplayMode currentState = RefreshListView.DisplayMode.PULL_DOWN_REFRESH;	// 头布局当前的状态, 默认为下拉状态
    ...
    public BasListView(Context context, AttributeSet attrs) {
        super(context,attrs);
        initHeaderView();
        initFooterView();
    }
...
//        addFooterView(footerView);
    ...
    private void refreshHeaderViewState() {
        if(currentState == RefreshListView.DisplayMode.PULL_DOWN_REFRESH) {	// 下拉刷新状态
            ivArrow.startAnimation(downAnimation);
            tvState.setText("下拉刷新");
        } else if(currentState == RefreshListView.DisplayMode.RELEASE_REFRESH) {	// 松开刷新
            ivArrow.startAnimation(upAnimation);
            tvState.setText("松开刷新");
        } else if(currentState == RefreshListView.DisplayMode.REFRESHING) {		// 正在刷新
    ...
    public void setAdapter(RebateAdapter adapter) {
    }
...
//            setSelection(getCount());	// 滚动到ListView的最后一条
    ...
    public void onRefreshFinish() {
        if(isLoadMoring) {
            ...
            mRefreshView.setPadding(0, -refreshViewHeight, 0, 0);
            currentState = RefreshListView.DisplayMode.PULL_DOWN_REFRESH;
    ...
    public boolean onTouchEvent(MotionEvent ev) {
        switch (ev.getAction()) {
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                if(currentState == RefreshListView.DisplayMode.REFRESHING) {	// 如果正在刷新中, 不做下拉的动作
                    break;
                }
                ...
                if(isEnablePullToRefresh
                    ...
                    if(currentState == RefreshListView.DisplayMode.PULL_DOWN_REFRESH
                            && paddingTop > 0) {		// 完全显示,并且当前的状态是下拉刷新状态
                        currentState = RefreshListView.DisplayMode.RELEASE_REFRESH;
                        refreshHeaderViewState();
                    } else if(currentState == RefreshListView.DisplayMode.RELEASE_REFRESH
            ...
            case MotionEvent.ACTION_UP:
                ...
                if(currentState == RefreshListView.DisplayMode.PULL_DOWN_REFRESH) {	// 下拉刷新状态
                    ...
                    currentState = RefreshListView.DisplayMode.REFRESHING;
                    refreshHeaderViewState();
